  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      ...
      if (isStory) {
        ...
        storyxhr.onload = function () {
          if (storyxhr.status === 200) {
            location.href =
              ...
              JSON.parse(storyxhr.responseText).id;
          } else {
            alert('failed to upload story');
          }
        };
        var _ = {
          img: JSON.parse(xhr.responseText).location,
        };
        ...
        document.img = JSON.parse(xhr.responseText).location;
      }
    }
  };
  xhr.open('POST', '/p/img');
...
window.addEventListener('load', function () {
  ...
  span.onclick = function () {
    ...
    xhr.onload = function () {
      if (xhr.status === 201) {
        location.href =
          'http://localhost:3001/p/' + JSON.parse(xhr.responseText).id;
  ...
  document
    .querySelectorAll('.fa-user')[0]
    .addEventListener('click', function (e) {
      var usernav = document.querySelectorAll('.usernav')[0];
      usernav.style.display =
        usernav.style.display === 'none' ? 'initial' : 'none';
    });
});
